  ...
  useEffect(() => {
    const fetchMenu = async () => {
      ...
      try {
        ...
        const allDishes = Array.isArray(response.data?.data)
          ? response.data.data
          : [];
        ...
        <div className="xl:mb-20 grid grid-cols-1 xl:px-16 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {loading ? (
            ...
            dishes.map((dish, index) => (
              ...
              >
                ...
                <div className="p-4 flex flex-col items-center text-center">
                  <h2 className="text-2xl font-barber tracking-wide">
                    {dish.title}
                  </h2>
                  <p className="text-lg mt-1 2xl:px-22 text-[#C20000]">
                    {dish.subtitle}
                  </p>
                </div>
              </motion.div>
            ...
            <p className="text-center col-span-3  text-[#C20000] text-lg">
              No {selectedType.toLowerCase()} dishes available in{" "}
              {selectedCategory}.
            </p>
          )}
        </div>
